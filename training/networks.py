...
class SongUNet(torch.nn.Module):
    def __init__(self,
        ...
        fourier_scale       = 0.02,         # Scale for the Fourier Embedding.
...
class DhariwalUNet(torch.nn.Module):
    def __init__(self,
        ...
        for level, mult in enumerate(channel_mult):
            res = img_resolution >> level
            res_dropout = dropout if res <= 16 else 0.
            block_kwargs['dropout'] = res_dropout
            if level == 0:
                cin = cout
                cout = model_channels * mult
                self.enc[f'{res}x{res}_conv'] = Conv2d(in_channels=cin, out_channels=cout, kernel=3, **init)
...
class EDMPrecond(torch.nn.Module):
    ...
    def forward(self, x, sigma, class_labels=None, force_fp32=False, **model_kwargs):
        ...
        c_out = (sigma - self.sigma_min) * self.sigma_data / ((sigma - self.sigma_min) ** 2 + self.sigma_data ** 2).sqrt()
